...
describe('/api/returns', () => {
    ...
    beforeEach(async () => { 
        ...
        await rental.save();    // Save to the database
    });

    afterEach(async () => { 
        await server.close(); 
        await Rental.remove({});    // Clean up the database
        await Movie.remove({});
    });
    ...
    it('should return 404 if rental not found with customerId/movieId', async () => {
        await Rental.remove({});
        const res = await exec();
    ...
    it('should set a return date if input is valid', async () => {
        await exec();
        // Reload the rental from db instead of just use the one in memory
        const rentalInDb = await Rental.findById(rental._id);
        // Check if current time - returned time is less than 10 secs
        const diff = new Date() - rentalInDb.dateReturned; 
        expect(diff).toBeLessThan(10 * 1000);
    });

    it('should set a rentalFee if input is valid', async () => {
        // Set the dateOut to be 7 days ago
